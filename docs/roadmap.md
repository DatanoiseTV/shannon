...
| `shannon ask` additional backends | Current backends (auto-discovered on localhost): Ollama, LM Studio, vLLM, llama.cpp server, LocalAI, LiteLLM, generic OpenAI-compatible. Planned: **opencode** (sst/opencode CLI; spawn as subprocess, feed our question, parse its answer), Anthropic native API (non-OpenAI-compat path), Gemini native, AWS Bedrock Converse API. Also: read environment-variable credentials (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GEMINI_API_KEY`, `AWS_PROFILE`) and `~/.config/shannon/models.toml` for aliasing. |
| Agent mode for `ask` | Extended tool set beyond read-only: `create_alert(rule)` persists a warning rule into a file, `generate_openapi(catalog, out)` runs the existing export, `explain_flow(sock_id)` gathers every event for a connection and summarises. Multi-turn conversation mode with history loaded from `~/.local/share/shannon/ask-history.jsonl`. |
...
| Protobuf schema brute-force | `shannon proto infer --samples dir/ --threads N --time 60s` multithread search for the most-likely `.proto` schema given a corpus of raw binary messages. Each worker tries a random permutation of (tag → wire-type → name-hint) mappings, decodes the whole corpus under that hypothesis, and scores by (bytes consumed / length of decoded-as-nested-messages / UTF-8 ratio of Len fields). Workers share a Bloom filter of tried permutations and a lock-free leaderboard; main thread prints the current best every N seconds. Termination conditions: time budget exhausted, leaderboard unchanged for M seconds, or all-fields-named hit. Emits: `inferred.proto` with best-fit definitions, plus a confidence table listing alternatives. |
